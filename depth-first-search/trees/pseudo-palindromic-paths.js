...
// DISCLAIMER: this solutions works for the example test cases but fails upon submission because one of their test cases
// is an extremely, ridiculously long test case
 ...
 */
 var pseudoPalindromicPaths  = function(root) {
    ...
    const dfs = (root, path = []) => {
        if(!root) return;
        const newPaths = [...path, root.val];
        if(!root.left && !root.right) {
            const freq = Array(10).fill(0);
            newPaths.forEach(num => freq[num]++);
            const odds = freq.reduce((prev, cur) => (cur % 2) + prev);
            if(odds < 2) return paths += 1;
        };
        dfs(root.left, newPaths);
        dfs(root.right, newPaths);
    };
    dfs(root);
    return paths;
};